...
describe('ProjectsListClient — sort controls', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockSearchParamsValue = new URLSearchParams();
    global.fetch = jest.fn().mockResolvedValue({
  ...
  it('renders Sort Direction select', async () => {
    ...
    const sortDirSelect = document.getElementById('projects-sort-dir') as HTMLSelectElement;
  ...
  it('defaults to updatedAt / desc sort', async () => {
    await act(async () => {
      render(<ProjectsListClient />);
    });
    ...
    await waitFor(() => {
      ...
      expect(initialCall).toContain('sortBy=updatedAt');
      expect(initialCall).toContain('sortDir=desc');
    });
  });
  ...
  it('reads sortBy and sortDir from URL search params on initial render', async () => {
    mockSearchParamsValue = new URLSearchParams('sortBy=status&sortDir=asc');
    ...
    await waitFor(() => {
      const calls = (global.fetch as jest.Mock).mock.calls as [string][];
      const initialCall = calls[0]?.[0] as string;
      expect(initialCall).toContain('sortBy=status');
